...
export default function PetRecords() {
  ...
  if (!medicalFormData.date || !medicalFormData.type || !medicalFormData.description) {
    alert("Please fill in all medical record fields");
    return;
  }
  ...
  // 1. Update pet's medical history
  setPets(prevPets => {
    return prevPets.map(pet => {
      if (pet.id === selectedPetForMedical) {
        petName = pet.name;
        return {
          ...pet,
          medicalHistory: [...(pet.medicalHistory || []), medicalFormData]
        };
      }
      return pet;
    });
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-2xl w-full shadow-2xl">
              ...
              <div className="mt-6 flex justify-end gap-4">
                ...
                <button onClick={handleAddMedicalRecord} className="px-6 py-3 bg-green-400 text-white rounded-3xl font-semibold hover:scale-105 transition-all">Add Record</button>
              </div>
            </div>
          </div>
        )}
